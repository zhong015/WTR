...
@property(nonatomic,copy)NSString *curintFilePath,*tmpPath;

@property(nonatomic,weak)WTREpubViewController *curintEpubVc;

@end

static id _s;
@implementation WTREPUB
{
    NSString *filetmpPath,*opfFilePath,*ncxFilePath,*coverFilePath,*htmlrootPath;
    EPUBParserW *parser;
    NSDictionary *epubFileInfo;
    NSArray *epubCatalog,*epubPageRef,*epubPageItem;
}

+(WTREPUB *)shareInstence
{
    @synchronized(self){
        if (_s==nil) {
...
-(id)init
{
    self=[super init];
    if (self) {
        self.tmpPath=[NSHomeDirectory() stringByAppendingString:@"/tmp"];
        
        parser=[[EPUBParserW alloc]init];
    }
    return self;
}
#pragma mark 加载文件
...
{
    return [[WTREPUB shareInstence] OpenEpubFile:filepath];
}
#pragma mark 关闭上次文件
+(void)closeCurintFile
{
    [[WTREPUB shareInstence] closeCurintFile];
}
-(void)closeCurintFile
{
    if (self.curintEpubVc&&self.curintEpubVc.view.window) {
        [self.curintEpubVc dismissViewControllerAnimated:YES completion:nil];
        self.curintEpubVc=nil;
    }
    self.htmlArray=[NSMutableArray array];
}
+(NSString *)filetmpPathWithFile:(NSString *)filepath
{
    return [[WTREPUB shareInstence].tmpPath stringByAppendingPathComponent:[[filepath lastPathComponent] stringByDeletingPathExtension]];
}
-(void)OpenEpubFile:(NSString *)filepath
{
    ...
    [self closeCurintFile];
  
    self.curintFilePath=filepath;
    filetmpPath=[self.tmpPath stringByAppendingPathComponent:[[filepath lastPathComponent] stringByDeletingPathExtension]];
    
    NSString *containerPath=[[filetmpPath stringByAppendingPathComponent:@"META-INF"] stringByAppendingPathComponent:@"container.xml"];
    
    if (![[NSFileManager defaultManager] fileExistsAtPath:containerPath]) {
        [[NSFileManager defaultManager] createDirectoryAtPath:filetmpPath withIntermediateDirectories:YES attributes:nil error:nil];
        
        [parser closeFile];
        
        [self openFilePath:filepath WithUnzipFolder:filetmpPath];
    }
    
    NSLog(@"%@",filetmpPath);
    
    opfFilePath=[parser opfFilePathWithManifestFile:containerPath WithUnzipFolder:filetmpPath];
    if (!opfFilePath) {
//        UIAlertViewShowF(@"当前文件损坏");
        return ;
    }
    htmlrootPath=[opfFilePath stringByDeletingLastPathComponent];
    
    ncxFilePath=[self ncxFilePathWithOpfFile:opfFilePath WithUnzipFolder:filetmpPath];
    
    coverFilePath=[parser coverFilePathWithOpfFile:opfFilePath WithUnzipFolder:filetmpPath];//封面
    
    epubFileInfo=[parser epubFileInfoWithOpfFile:opfFilePath];//文件基本信息
    
    epubCatalog=[self epubCatalogWithNcxFile:ncxFilePath];//[parser epubCatalogWithNcxFile:ncxFilePath];//目录
    
    epubPageRef=[parser epubPageRefWithOpfFile:opfFilePath];//html页码索引 所有的html文件
    
    epubPageItem=[parser epubPageItemWithOpfFile:opfFilePath];//页码信息 用于 由html页码索引 来得到html文件名
    
    if (epubPageRef&&epubPageRef.count>0) {
        for (NSString *htmlid in epubPageRef) {
            for (NSDictionary *dic in epubPageItem) {
                NSString *idstr=[dic objectForKey:@"id"];
                if ([idstr isEqualToString:htmlid]) {
                    
                    WTREpubHtmlFile *file=[[WTREpubHtmlFile alloc]init];
                    
                    file.htmlid=idstr;
                    NSString *href=[dic objectForKey:@"href"];
                    file.filePath=[htmlrootPath stringByAppendingPathComponent:href];
                    file.index=self.htmlArray.count;
                    
                    if (epubCatalog&&epubCatalog.count>0) {
                        for (NSDictionary *diccat in epubCatalog) {
                            NSString *catsrcstr=[diccat objectForKey:@"src"];
                            if (catsrcstr&&[catsrcstr isEqualToString:href]) {
                                file.htmltext=[diccat objectForKey:@"text"];
                                break;
                            }
                        }
                    }
                    
                    if (!ISString(file.htmltext)) {
                        file.htmltext=idstr;
                        NSString *str=[NSString stringWithContentsOfFile:file.filePath encoding:NSUTF8StringEncoding error:nil];
                        if (ISString(str)) {
                            NSRange rabd=[str rangeOfString:@"<body>"];
                            if (rabd.length>0) {
                                str=[str substringFromIndex:rabd.location];
                                str=[WTR jianHuaHtml:str];
                                if (!ISString(str)) {
                                    break;
                                }
                            }else{
    ...
    [self performSelector:@selector(showvcview) withObject:nil afterDelay:0.1];
...
-(void)showvcview
{
    if (self.htmlArray.count>0) {
        WTREpubViewController *epubvc=[[WTREpubViewController alloc]init];
        epubvc.epubpath=self.curintFilePath;
        self.curintEpubVc=epubvc;
        
        self.curintEpubVc.titleName=[epubFileInfo objectForKey:@"dc:title"];
        
        self.curintEpubVc.htmlArray=self.htmlArray;
        
        AppDelegate *app=(AppDelegate *)[UIApplication sharedApplication].delegate;
        [app.window.rootViewController presentViewController:self.curintEpubVc animated:YES completion:nil];
    }
}
...
-(int)unzipWithFileFullPath:(NSString*)fileFullPath WithUnzipFolder:(NSString *)unzipFolder
{
    ...
    //开始解压
    ZipArchive2* za = [[ZipArchive2 alloc] init];
    za.delegate=self;
    ...
    if( [za UnzipOpenFile:fileFullPath])
    {
        ...
        if ( ! bUnZip)
        {
            NSLog(@"%@",[NSString stringWithFormat:@"Error while unzipping the epub ,file=%@",fileFullPath]);
    ...
    if (xmlData)
    {
        ...
        if ([err code] == 0)
        {
            ...
            if ([itemsArray count]<1)
            {
                ...
                for (GDataXMLElement *ele1 in arr) {
                    
                    NSArray *arr2=[ele1 children];
                    if (arr2.count>0) {
                        for (GDataXMLElement *ele2 in arr2) {
                            GDataXMLNode *cnode=[ele2 attributeForName:@"id"];
                            if (cnode) {
                                if ([cnode.stringValue rangeOfString:@"ncx"].length>0) {
                                    itemsArray=@[ele2];
                                    break;
                                }
                            }
                    ...
                    GDataXMLNode *cnode=[ele1 attributeForName:@"id"];
                    if (cnode) {
                        if ([cnode.stringValue rangeOfString:@"ncx"].length>0) {
                            itemsArray=@[ele1];
                            break;
...
/*
 ...
 @interface EPUBParser()<ZipArchiveDelegate>
 ...
 {
 #ifdef DEBUG
 NSLog(@"ZipArchiveDelegate msg=%@",msg);
 #endif
 }
 ////////////////////////////////////////////////////////////////////////////////
 ///////////////////      公共方法     /////////////////////////////////
